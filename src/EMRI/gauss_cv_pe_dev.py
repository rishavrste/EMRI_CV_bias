"""0PA + simple (p,e-trajectory) deviation: CV -> (stall) NM(500) -> CV, all 3 EMRI cases.

The deviation is the current SuperKludgeFlux one: a multiplicative rescaling of the
adiabatic p, e trajectory,
    pdot -> (1 + eta * del_0_p) * pdot ,   edot -> (1 + eta * del_0_e) * edot ,
applied when deviation_included, before the 1PA/2PA blocks.  In additional_args:
    [0]chi2 [1]evolve_1PA [2]evolve_primary [3]evolve_2PA [4]deviation_included
    [5]del_0_p [6]del_0_e
so dev_1 -> del_0_p (idx 5), dev_2 -> del_0_e (idx 6).

Each case starts from its CV-refined 0PA best fit with dev_1 = dev_2 = 0 and tries to
improve the overlap with the 1PA signal via adaptive-LM CV; if CV stalls below the overlap
limit, one 500-step Nelder-Mead escape then one more CV.

Run:  python gauss_cv_pe_dev.py
"""
...
# --- deviation wiring: del_0_p (idx 5), del_0_e (idx 6) --------------------
DEV_NAME = "simple p,e"


def dev_tail(dev_1, dev_2):
    """additional_args slots after deviation_included: [del_0_p, del_0_e]."""
    return [dev_1, dev_2]
...
def build_context(case):
    ...
    def make0(vec):                                    # 0PA + p,e-deviation template
...
def run_case(case):
    ...
    return dict(name=case["name"], snr=ctx["snr"], ov_start=float(ctx["ov"](case["start"])),
                ov_final=float(ctx["ov"](theta)), params=theta.copy())
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"model = 0PA + {DEV_NAME} deviation  (pdot/edot multiplicative, del_0_p/del_0_e)")
    print(f"{'case':6} {'SNR':>7} {'ov@start':>11} {'ov@final':>11} {'improved?':>10}")
    for r in results:
        imp = "YES" if r["ov_final"] > r["ov_start"] + 1e-8 else "no"
        print(f"{r['name']:6} {r['snr']:>7.2f} {r['ov_start']:>11.7f} {r['ov_final']:>11.7f} {imp:>10}")
