"""EMRI diverse grid points (set 2): 0PA + PN and 0PA + simple, SEEDED FROM THE 0PA BEST FIT.

Six grid points (a, e0): idx3 (0.5, 0.1), idx5 (-0.9, 0.2), idx11 (-0.5, 0.3),
                         idx17 (0.0, 0.4), idx20 (-0.9, 0.5), idx24 (0.9, 0.5).

For each point we run BOTH deviation models, each started from that point's 0PA best fit
(9 params, from gauss_cv_emri_grid_diverse2.py's from_MAP 0PA run) with dev_1 = dev_2 = 0.
Starting exactly at the 0PA optimum, the CV climb can only move uphill, so every result is
guaranteed >= the 0PA overlap (removes the from_MAP seeding artifact and unlocks PN gains
the MAP seed misses at well-fit points).
...
Run:  python gauss_cv_emri_grid_diverse2_dev_from_0pa.py
"""
...
def _sig(a, p0, e0, dist):
    return {"m1": 1e6, "m2": 10.0, "a": a, "p0": p0, "e0": e0, "xI0": 1.0, "dist": dist,
...
SIG = {
    "idx3":  _sig(0.5,  9.987641937579998,  0.1, 9.414980595910297),
    "idx5":  _sig(-0.9, 13.122559320333108, 0.2, 3.0324274273280873),
    "idx11": _sig(-0.5, 12.255354927771608, 0.3, 4.4616428166698565),
    "idx17": _sig(0.0,  11.104224019622757, 0.4, 6.926568915250337),
    "idx20": _sig(-0.9, 13.302718414742197, 0.5, 4.121669070558761),
    "idx24": _sig(0.9,  8.926976081703573,  0.5, 14.334976856664628),
}

# start_0pa = the 0PA best fit (9 params) from the diverse2 from_MAP 0PA run.
POINTS = [
    dict(name="idx3", a=0.5, e0=0.1, ov_0pa=0.9998803215017041, signal_param=SIG["idx3"],
         start_0pa=[1000016.5703865323, 10.000071025888891, 0.5000316866018595, 9.987503844327515,
                    0.09998769397715761, 1.0462910749294652, 0.784279599893177,
                    0.09693573660083404, 0.20728489161257063]),
    dict(name="idx5", a=-0.9, e0=0.2, ov_0pa=0.9999852981874162, signal_param=SIG["idx5"],
         start_0pa=[1000059.6505883028, 10.000080853079092, -0.8998281858261038, 13.121963034921146,
                    0.19998768770725073, 1.0460621365796432, 0.7854369417783729,
                    0.10476002189080197, 0.2892320560284472]),
    dict(name="idx11", a=-0.5, e0=0.3, ov_0pa=0.99998289406324, signal_param=SIG["idx11"],
         start_0pa=[1000060.5368472723, 10.000114359422101, -0.49985470388196057, 12.254810227922711,
                    0.29999053048712404, 1.0462114850678637, 0.7853148752520462,
                    0.10485747877629341, 0.29067577218548135]),
    dict(name="idx17", a=0.0, e0=0.4, ov_0pa=0.9999627910169843, signal_param=SIG["idx17"],
         start_0pa=[1000060.6927734533, 10.000173785230864, 0.00010997550556886966, 11.103759246986689,
                    0.39999158931409734, 1.0461912156843725, 0.7850739671909345,
                    0.10515920523946738, 0.2894057671434577]),
    dict(name="idx20", a=-0.9, e0=0.5, ov_0pa=0.9999946735954659, signal_param=SIG["idx20"],
         start_0pa=[999892.3343376793, 9.999638039171202, -0.9001763442473382, 13.303408920721337,
                    0.5000133497489407, 1.0473074477641067, 0.7854761431857625,
                    0.10411437672087236, 0.30096275630888714]),
    dict(name="idx24", a=0.9, e0=0.5, ov_0pa=0.9954962715024418, signal_param=SIG["idx24"],
         start_0pa=[1000011.1845337861, 10.000191590897293, 0.9000128462239749, 8.926937860476121,
                    0.4999958475170689, 1.045804085870438, 0.7818325211083005,
                    0.28505609091200323, 0.184787851472214]),
]
...
JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "results_emri_grid_diverse2_dev_from_0pa.json")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("EMRI diverse set 2: 0PA + PN / simple, seeded from 0PA best fit")
