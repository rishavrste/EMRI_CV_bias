"""EMRI diverse grid points: 0PA + PN and 0PA + simple, SEEDED FROM THE 0PA BEST FIT.

Four grid points (a, e0): idx0 (-0.9, 0.1), idx6 (-0.5, 0.2), idx12 (0.0, 0.3), idx18 (0.5, 0.4).

For each point we run BOTH deviation models, each started from that point's 0PA best fit
(9 params, from gauss_cv_emri_grid_diverse.py's from_MAP 0PA run) with dev_1 = dev_2 = 0.
Starting exactly at the 0PA optimum, the CV climb can only move uphill, so every result is
guaranteed >= the 0PA overlap (removes the "deviation worse than 0PA" convergence artifact).

    4 points x 2 models (PN, simple) = 8 CV climbs.
...
Run:  python gauss_cv_emri_grid_diverse_dev_from_0pa.py
"""
...
# start_0pa = the 0PA best fit (9 params) from the diverse run's from_MAP 0PA result.
POINTS = [
    dict(name="idx0", ov_0pa=0.999986147968984,
         ...
         start_0pa=[1000044.7720278731, 10.00005294590481, -0.8998601592219645,
    ...
    dict(name="idx6", ov_0pa=0.9999843699680888,
         signal_param={"m1": 1e6, "m2": 10.0, "a": -0.5, "p0": 12.218336776485106, "e0": 0.2,
                       ...
                       "phiK": 0.5235987755982988, "Phi_phi0": 0.1, "Phi_theta0": 0.2,
                       "Phi_r0": 0.3, "dev_1": 0.0, "dev_2": 0.0},
         start_0pa=[1000053.6810327651, 10.000072690976687, -0.49986579385681296,
    ...
    dict(name="idx12", ov_0pa=0.9999614279079387,
         signal_param={"m1": 1e6, "m2": 10.0, "a": 0.0, "p0": 11.105928547343611, "e0": 0.3,
                       ...
                       "phiK": 0.5235987755982988, "Phi_phi0": 0.1, "Phi_theta0": 0.2,
                       "Phi_r0": 0.3, "dev_1": 0.0, "dev_2": 0.0},
         start_0pa=[1000051.4782712947, 10.000104989414359, 9.899365889262895e-05,
    ...
    dict(name="idx18", ov_0pa=0.9997665959425159,
         signal_param={"m1": 1e6, "m2": 10.0, "a": 0.5, "p0": 9.941538910913811, "e0": 0.4,
                       ...
                       "phiK": 0.5235987755982988, "Phi_phi0": 0.1, "Phi_theta0": 0.2,
                       "Phi_r0": 0.3, "dev_1": 0.0, "dev_2": 0.0},
         start_0pa=[1000043.1904769371, 10.00017873801177, 0.5000583381308088,
...
JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "results_emri_grid_diverse_dev_from_0pa.json")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("EMRI diverse: 0PA + PN / simple, seeded from 0PA best fit")
