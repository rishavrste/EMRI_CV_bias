"""EMRI grid (idx4, idx14) 0PA + SIMPLE-PE deviation, CV from TWO starts.
...
For each case CV (adaptive-LM; if it stalls, one 1000-step Nelder-Mead escape then one more
CV) is run from BOTH:
    * from_MAP    : the 0PA-vs-2PA recovered MAP        (+ dev=0)
    * from_0PAcv  : our CV-optimized 0PA best fit       (+ dev=0)
Both endpoints are reported so you can see which start wins and whether the pe deviation helps.

All runs: dt=10, T=2.5, nchannels=3, no noise, chi2 (secondary spin) = 0.95.
Requires SuperKludge_r on the 'dev_a_pe' branch.

Run:  python gauss_cv_emri_grid_simple_pe.py
"""
...
CASES = [
    dict(name="idx4", dt=10.0, T=2.5, chi2=0.95, ov_0pa=0.9994332857784283,
         ...
         start_map=[9.9999312902e+05, 1.0000034144e+01, 8.9999831452e-01, 9.0868360349e+00,
                    9.9999256695e-02, 1.0471975512e+00, 7.8539816340e-01, 1.0118033613e-01,
                    2.9999226055e-01],
         start_0pacv=[999991.3396361339, 10.000055320096262, 0.8999987866684177,
                      9.086848640310997, 0.0999981971941262, 1.0468355740845003,
                      0.7845611461709062, 0.11572449148755387, 0.08574687812320049]),
    dict(name="idx14", dt=10.0, T=2.5, chi2=0.95, ov_0pa=0.9969378906095648,
         signal_param={"m1": 1e6, "m2": 10.0, "a": 0.9, "p0": 9.048439858487455, "e0": 0.3,
                       "xI0": 1.0, "dist": 14.134243467540808, "qS": 1.0471975511965976,
                       "phiS": 0.7853981633974483, "qK": 0.6283185307179586,
                       "phiK": 0.5235987755982988, "Phi_phi0": 0.1, "Phi_theta0": 0.2,
                       "Phi_r0": 0.3, "dev_1": 0.0, "dev_2": 0.0},
         start_map=[9.9999725137e+05, 1.0000063704e+01, 9.0000202970e-01, 9.0484577531e+00,
                    2.9999822161e-01, 1.0471975512e+00, 7.8539816340e-01, 1.0347980230e-01,
                    3.0892018459e-01],
         start_0pacv=[999999.5269184131, 10.000103696234472, 0.9000052110011653,
                      9.048450475798832, 0.2999967555902941, 1.046231851622652,
                      0.7821788932741249, 0.20856486638833024, 0.12136562724626632]),
]
...
JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         f"results_emri_grid_{DEV_NAME}.json")


def _jsonable(results):
    out = {"model": f"0PA + {DEV_NAME} deviation", "deviation": DEV_NAME, "system": "EMRI_grid",
           "n_params": len(params_to_infer), "param_names": params_to_infer,
           "generated_utc": datetime.now(timezone.utc).isoformat(), "cases": []}
    for r in results:
        entry = {"name": r["name"], "snr": r["snr"], "ov_0pa": r["ov_0pa"]}
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"model = 0PA + {DEV_NAME} deviation  (EMRI grid, 11 params)")
    print(f"{'case':6} {'SNR':>8} {'start':>11} {'ov@start':>11} {'ov@CV':>11} {'chi2':>11} "
          f"{'ov@0PA':>10}")
    for r in results:
        for label in ("from_MAP", "from_0PAcv"):
            d = r[label]
            print(f"{r['name']:6} {r['snr']:>8.2f} {label:>11} {d['ov_start']:>11.6f} "
                  f"{d['ov_final']:>11.6f} {d['chi2']:>11.3e} {r['ov_0pa']:>10.6f}")
