"""EMRI 0PA + SIMPLE deviation, refined with CV from TWO starts.
...
1000-step Nelder-Mead escape then one more CV) is run from BOTH:
    ...
    * start_dev : the NM 0PA+simple-deviation best fit
...
Run:  python gauss_cv_emri_simple_dev.py
"""
...
CASES = [
    dict(name="idx0", dt=5.0, T=1.0, chi2=0.0,
         zeropa_overlap=0.99999443, nm_dev_overlap=0.9982902463962956,
         ...
         start_dev=[1000389.2286868211, 10.002302094121655, 0.9002559808895736,
                    7.498384772547924, 0.4999476943146139, 0.7757447402149541,
                    1.009770680805333, 0.9261425036103994, 0.5330729116176761,
                    -0.6656728603075766, -0.000128886594314964]),
    dict(name="idx9", dt=10.0, T=2.5, chi2=0.95,
         zeropa_overlap=0.99818007, nm_dev_overlap=0.9980454212618244,
         ...
         start_dev=[999997.5559023082, 10.000093556782003, 0.9000035584838859,
                    9.074162056060484, 0.19999563523999941, 1.0465843782100788,
                    0.7827052467660807, 0.15799142679339725, 0.09085725447766498,
                    -0.0002668271720200931, 0.8767524748074431]),
    dict(name="idx13", dt=10.0, T=2.5, chi2=0.95,
         zeropa_overlap=None, nm_dev_overlap=0.9998873034358231,
         ...
         start_dev=[1000130.9108746902, 10.002079248419466, 0.5001368726099394,
                    9.969958082676865, 0.29995507157393553, 1.0476215162913225,
                    0.7839139980005259, 0.12508756604949617, 0.28635640030194504,
                    -13.820665445633262, 6.7718384578868545]),
]
