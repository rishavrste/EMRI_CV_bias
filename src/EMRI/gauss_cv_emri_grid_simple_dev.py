"""EMRI grid (least-biased) 0PA + SIMPLE deviation: 1PA injection -> 0PA+dev MAP via LM-damped CV.

Least-biased grid points (a=+0.9 column):  idx4 (e0=0.1), idx14 (e0=0.3).
...
Signal = 1PA injection; template = 0PA + deviation (11 params). The CV climb STARTS FROM THE
INJECTION (intrinsic+phase params, dev_1=dev_2=0) and ascends to the 0PA+deviation MLP:
adaptive-LM CV; if it stalls, one 1000-step Nelder-Mead escape, then one more CV.

All runs: dt=10, T=2.5, nchannels=3, no noise, chi2 (secondary spin) = 0.95.
Requires SuperKludge_r on the 'hybrid' branch.

Run:  python gauss_cv_emri_grid_simple_dev.py
"""
...
# 0PA-vs-2PA recovered MAP (from the critical-SNR grid) -- used as the CV start (dev=0 appended).
START_MAP = {
...
# --- per-case driver: start FROM THE INJECTION (dev = 0) --------------------
def run_case(case):
    ctx = build_context(case)
    start = np.array(list(START_MAP[case["name"]]) + [0.0, 0.0], dtype=float)   # MAP seed + dev=0
...
def _jsonable(results):
    out = {"model": f"0PA + {DEV_NAME} deviation", "deviation": DEV_NAME, "system": "EMRI_grid",
           "start": "0PA-vs-2PA MAP (dev=0)", "n_params": len(params_to_infer),
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"model = 0PA + {DEV_NAME} deviation  (EMRI grid, 11 params, start = 0PA-vs-2PA MAP)")
