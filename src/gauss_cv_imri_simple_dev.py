"""IMRI 0PA + SIMPLE deviation, refined with CV from TWO starts.

Simple deviation (hybrid-branch SuperKludgeFlux): multiplicative Edot/Ldot flux rescaling,
    additional_args[7]=del_0_p=dev_1, [8]=del_0_e=dev_2, deviation_included=True.

For each IMRI case the CV method (adaptive-LM; if it stalls below the overlap limit, one
1000-step Nelder-Mead escape then one more CV) is run from BOTH:
    * start_0pa : the CV-optimized 0PA best fit with dev_1 = dev_2 = 0
    * start_dev : the earlier NM/DE 0PA+simple-deviation best fit
and both endpoints are reported so you can see which start wins.

All runs: dt=5, T=1.0, nchannels=3, no noise, chi2 (secondary spin) = 0.95.

Run:  python gauss_cv_imri_simple_dev.py
"""
...
def dev_tail(dev_1, dev_2):
    """[C_p, C_e, del_0_p, del_0_e] slots of additional_args for the simple deviation."""
...
CASES = [
    dict(name="idx0", dt=5.0, T=1.0, chi2=0.95, nm_dev_overlap=0.992155,
         ...
         start_0pa=[1.00176199e+06, 4.99511288e+03, 7.03962258e-01, 2.49674581e+01,
                    2.49798441e-01, 7.86137900e-01, 1.00491369e+00, 9.89953388e-01,
                    6.42950551e-01, 0.0, 0.0],
         start_dev=[1002549.4845, 4971.1853, 0.704930, 24.95886, 0.249606, 0.793182,
                    1.005330, 1.184933, 0.615905, 1.019573, 2.170108]),
    dict(name="idx1", dt=5.0, T=1.0, chi2=0.95, nm_dev_overlap=0.999792,
         ...
         start_0pa=[9.98144092e+05, 1.00129945e+03, -4.99651156e-01, 2.49922458e+01,
                    9.98840371e-02, 1.05172816e+00, 7.90279570e-01, 6.30880364e+00,
                    4.49650077e-01, 0.0, 0.0],
         start_dev=[1001501.4641, 1000.0808, -0.494049, 24.93842, 0.099380, 1.047179,
                    0.786461, 6.442446, 0.193227, -0.345109, 10.955755]),
    dict(name="idx16", dt=5.0, T=1.0, chi2=0.95, nm_dev_overlap=0.997521,
         signal_param={"m1": 1e6, "m2": 1.0e3, "a": -0.50, "p0": 24.6855302, "e0": 0.40,
                       "xI0": 1.0, "dist": 39.7010887, "qS": 1.04719755, "phiS": 0.785398163,
                       "qK": 0.628318531, "phiK": 0.523598776, "Phi_phi0": 0.1,
                       "Phi_theta0": 0.2, "Phi_r0": 0.3, "dev_1": 0.0, "dev_2": 0.0},
         start_0pa=[1.00121834e+06, 9.99719120e+02, -4.94575948e-01, 2.46561703e+01,
                    3.99915754e-01, 1.05097533e+00, 7.80191278e-01, 2.14580670e-01,
                    -6.08369012e+00, 0.0, 0.0],
         start_dev=[1001441.0085, 999.4568, -0.494132, 24.65378, 0.399867, 1.047184,
                    0.780949, 0.423076, -6.059404, 0.354552, 0.788409]),
]
...
def cv_from(ctx, name, label, start):
    ...
    return dict(ov_start=float(ctx["ov"](start)), ov_final=float(ctx["ov"](theta)),
                params=theta.copy(), start=np.array(start, dtype=float))


# --- per-case driver: run CV from BOTH starts -------------------------------
def run_case(case):
    ctx = build_context(case)
    out = {"name": case["name"], "snr": ctx["snr"], "nm_dev_overlap": case["nm_dev_overlap"]}
...
def _jsonable(results):
    ...
    for r in results:
        entry = {"name": r["name"], "snr": r["snr"], "nm_dev_overlap": r["nm_dev_overlap"]}
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"model = 0PA + {DEV_NAME} deviation  (IMRI, 11 params)")
    print(f"{'case':6} {'SNR':>8} {'start':>10} {'ov@start':>11} {'ov@CV':>11} {'ov@NMdev':>11}")
    for r in results:
        for label in ("from_0PA", "from_NMdev"):
            d = r[label]
            print(f"{r['name']:6} {r['snr']:>8.2f} {label:>10} {d['ov_start']:>11.6f} "
                  f"{d['ov_final']:>11.6f} {r['nm_dev_overlap']:>11.6f}")
