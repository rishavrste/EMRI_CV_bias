"""IMRI 0PA + SIMPLE-PE deviation, refined with CV from the 0PA best fit.
...
Only 0PA optimized points are available for this branch, so for each IMRI case the CV method
...
Run:  python gauss_cv_imri_simple_pe.py
"""
...
CASES = [
    dict(name="idx0", dt=5.0, T=1.0, chi2=0.95,
    ...
    dict(name="idx1", dt=5.0, T=1.0, chi2=0.95,
    ...
    dict(name="idx16", dt=5.0, T=1.0, chi2=0.95,
...
def _jsonable(results):
    ...
    for r in results:
        d = r["from_0PA"]
        out["cases"].append({
            "name": r["name"], "snr": r["snr"],
            "from_0PA": {"ov_start": d["ov_start"], "ov_final": d["ov_final"],
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"model = 0PA + {DEV_NAME} deviation  (IMRI, 11 params, start = 0PA best fit)")
    print(f"{'case':6} {'SNR':>8} {'ov@start':>11} {'ov@CV':>11} {'gain':>11}")
    for r in results:
        d = r["from_0PA"]
        print(f"{r['name']:6} {r['snr']:>8.2f} {d['ov_start']:>11.6f} "
              f"{d['ov_final']:>11.6f} {d['ov_final'] - d['ov_start']:>11.2e}")
