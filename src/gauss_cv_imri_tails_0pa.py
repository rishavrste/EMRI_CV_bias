"""0PA fit of the IMRI-tails cases: refine the DE/NM 0PA best fit with CV.
...
adaptive-LM CV; if CV stalls below the overlap limit, one Nelder-Mead escape, then one
more CV.
...
Run:  python gauss_cv_imri_tails_0pa.py
"""
...
# start = the 0PA best fit (9 params) reported for each IMRI-tails case.
CASES = [
    dict(name="pt4", dt=10.0, T=0.25, chi2=0.95, nm_overlap=0.999865,
         ...
         start=[1000023.7578, 9988.3422, 0.899003, 29.274076, 0.100697,
                1.047049, 0.785082, 0.253958, 0.109638]),
    dict(name="pt20", dt=10.0, T=0.25, chi2=0.95, nm_overlap=0.997869,
         signal_param={"m1": 1e6, "m2": 1e4, "a": -0.9, "p0": 30.522071, "e0": 0.5,
                       "xI0": 1.0, "dist": 94.592671, "qS": 1.04719755, "phiS": 0.78539816,
                       "qK": 0.62831853, "phiK": 0.52359878, "Phi_phi0": 0.1,
                       "Phi_theta0": 0.2, "Phi_r0": 0.3, "dev_1": 0.0, "dev_2": 0.0},
         start=[1012624.1991, 9966.7874, -0.835534, 30.153167, 0.499398,
                1.044398, 0.786418, 0.173735, 0.238378]),
]
...
# --- per-case driver: CV -> (only if it stalls) NM -> CV --------------------
def run_case(case):
    ...
    return dict(name=case["name"], snr=ctx["snr"], ov_start=float(ctx["ov"](case["start"])),
                ...
                nm_overlap=case["nm_overlap"], start=np.array(case["start"], dtype=float),
                params=theta.copy())


JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                         "results_imri_tails_0pa.json")


def _jsonable(results):
    out = {"model": "0PA", "system": "IMRI_tails", "n_params": len(params_to_infer),
           "param_names": params_to_infer,
           "generated_utc": datetime.now(timezone.utc).isoformat(), "cases": []}
    for r in results:
        out["cases"].append({
            "name": r["name"], "snr": r["snr"], "nm_overlap": r["nm_overlap"],
