"""IMRI-tails 0PA + SIMPLE deviation, refined with CV from TWO starts.
...
For each tails case the CV method (adaptive-LM; if it stalls below the overlap limit, one
1000-step Nelder-Mead escape then one more CV) is run from BOTH:
    * start_0pa : the DE/NM 0PA best fit with dev_1 = dev_2 = 0
    * start_dev : the DE/NM 0PA+simple-deviation best fit
and both endpoints are reported so you can see which start wins.  The external DE/NM
reference overlaps (0PA and 0PA+deviation) are carried through for comparison.
...
Run:  python gauss_cv_imri_tails_simple_dev.py
"""
...
CASES = [
    dict(name="pt4", dt=10.0, T=0.25, chi2=0.95,
         zeropa_overlap=0.999865, nm_dev_overlap=0.999879,
         ...
         start_0pa=[1000023.7578, 9988.3422, 0.899003, 29.274076, 0.100697,
                    1.047049, 0.785082, 0.253958, 0.109638, 0.0, 0.0],
         start_dev=[999752.6006, 9988.2954, 0.898720, 29.278201, 0.100793,
                    1.046883, 0.786152, 0.187706, 0.021135, 0.001018, -0.905270]),
    dict(name="pt20", dt=10.0, T=0.25, chi2=0.95,
         zeropa_overlap=0.997869, nm_dev_overlap=0.999206,
         signal_param={"m1": 1e6, "m2": 1e4, "a": -0.9, "p0": 30.522071, "e0": 0.5,
                       "xI0": 1.0, "dist": 94.592671, "qS": 1.04719755, "phiS": 0.78539816,
                       "qK": 0.62831853, "phiK": 0.52359878, "Phi_phi0": 0.1,
                       "Phi_theta0": 0.2, "Phi_r0": 0.3, "dev_1": 0.0, "dev_2": 0.0},
         start_0pa=[1012624.1991, 9966.7874, -0.835534, 30.153167, 0.499398,
                    1.044398, 0.786418, 0.173735, 0.238378, 0.0, 0.0],
         start_dev=[1018135.3398, 10022.9935, -0.822283, 30.064854, 0.498844,
                    1.041702, 0.781380, 0.098381, 0.329293, -0.597271, -0.343141]),
]
...
def main():
    ...
    print("\n" + "=" * 70)
    print(f"model = 0PA + {DEV_NAME} deviation  (IMRI tails, 11 params)")
    print(f"{'case':6} {'SNR':>8} {'start':>10} {'ov@start':>11} {'ov@CV':>11} {'chi2':>11} "
          f"{'ov@0PA':>10} {'ov@NMdev':>10}")
    for r in results:
        for label in ("from_0PA", "from_NMdev"):
            d = r[label]
            print(f"{r['name']:6} {r['snr']:>8.2f} {label:>10} {d['ov_start']:>11.6f} "
                  f"{d['ov_final']:>11.6f} {d['chi2']:>11.3e} {r['zeropa_overlap']:>10.6f} "
                  f"{r['nm_dev_overlap']:>10.6f}")
